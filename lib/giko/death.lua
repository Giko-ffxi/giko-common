...
death.get_tod = function(name)
    ...
    local tod = mob ~= nil and common.in_array_key(death.memory, mob.names.nq[1]) and death.memory[mob.names.nq[1]] or (mob ~= nil and cache.get(death.cache, mob.names.nq[1]))
...
death.set_tod = function(name, gmt, day)
    ...
    if mob ~= nil then
        ...
        return cache.set(death.cache, mob.names.nq[1], death.memory[mob.names.nq[1]])
