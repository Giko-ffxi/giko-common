...
local death   = { cache = string.format('%s\\..\\giko-cache\\cache\\giko.monster.tod.csv', _addon.path) }

death.get_tod = function(name)

    local mob = monster.get(name)
    local tod = mob ~= nil and cache.get(death.cache, mob.names.nq[1])

    if mob ~= nil and tod ~= nil then
        return json:decode(tod)
    end
...
death.set_tod = function(name, gmt, day)
    ...
    if mob ~= nil then
        return cache.set(death.cache, mob.names.nq[1], json:encode({name = mob.names.nq[1], gmt = gmt, day = day, created_at = os.time()}))
    end
...
death.set_day = function(name, day)
    ...
    if mob ~= nil and tod ~= nil then
        return cache.set(death.cache, mob.names.nq[1], json:encode({name = tod.name, gmt = tod.gmt, day = day, created_at = os.time()}))
    end
